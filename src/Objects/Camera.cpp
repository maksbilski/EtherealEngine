...
Camera::Camera(GLFWwindow* window, World* world, glm::vec3 position)
    :   m_Window(window), m_World(world), m_Position(position) 
{
    m_JumpTimer = 0.0f;
    m_Gravity = 0.0f;
...
void Camera::computeWalkingVectors()
{
    ...
    m_ForwardVec = glm::rotate(glm::vec3(0.0f, 0.0f, -1.0f), m_LookAngleY, m_UpVec);
...
void Camera::controlMouseInput(float deltaTime)
{
    ...
    if(m_TargetLookAngleX > maxLookPitch) m_TargetLookAngleX = maxLookPitch;
    if(m_TargetLookAngleX < -maxLookPitch) m_TargetLookAngleX = -maxLookPitch;

    m_OldMouseX = mouseX;
    m_OldMouseY = mouseY;

    m_LookAngleX = m_LookAngleX + (m_TargetLookAngleX - m_LookAngleX) * deltaTime * turnSpeed;
    m_LookAngleY = m_LookAngleY + (m_TargetLookAngleY - m_LookAngleY) * deltaTime * turnSpeed;
...
void Camera::computeCameraOrientation()
{
    ...
    // Compute the look position by adding the forward vector to the camera position
